				      ...
				      bool force_reclaim)
{
	...
	cond_resched();
         ...
         * Date: 8/2/2017, 3pm
         * Function: Finds whether a page is in a given VMA of the current task.
                Will be used for analyzing spacial locality. Eventually,
        ...
        ***********************/
        ...
        int x, i = 0;
        struct vm_area_struct *vma[SIZE];        //For VMA's in
side processes.
        int pages_in_vma[SIZE];  //Number of pages per corresponding VM
A
        struct task_struct *task = current;     //For proceses
        for_each_process(task) {        //Selects a sample of VMA's before while loop.
                if ((i < SIZE) && (task->mm != NULL)) {
                	vma[i++] = task->mm->mmap;
			while (i < SIZE) {	//Iterate through VMA's per task.
	 ...
	 */
		for (i = 0; i < x; i++) {
			if (page_address_in_vma(page, vma[i]) != -EFAULT) {
				pages_in_vma[i]++;
			}
		}
	/* End of section B */
...
keep:
	...
	for (i = 0; i < x; i++) {	//Print results after while loop.
		if ((0 < ID && ID < SIZE) && (pages_in_vma[i] != 0)) {
			/* Must have a non-zero number of pages,
			 * positive ID under SIZE.
			 */
			printk(KERN_ALERT "ID %i -> VMA %i: %i pages\n", ID, i, pages_in_vma[i]);
		}
	}
	/* End of SECTIONS */
